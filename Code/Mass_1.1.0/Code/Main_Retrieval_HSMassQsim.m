function Main_Retrieval_HSMassQsim(filepath, Paras)
% MassQsim method
...
% if ~strcmpi(methodname, 'HSMassQsim')
...
load(strcat(localpath, 'hsmassdist_', dataname, '_', num2str(NumTree), '_', num2str(NumSub), '_', num2str(NumDim), '.mat'));

[ins_num, class_num] = size(Labels);
Pre_Distance = sqrt(Pre_Distance);
...
for class_index = 1:class_num
    for query_index = 1:query_num
        ...
        % Only a Query
        Query_Score = - Pre_Distance(query, :);
        Query_Time = 0;
        ...
        for fbp_index = 1:fb_process
            ...
            for fbr_index = 2:fb_round + 1
                ...
                [Curt_Score, Time{fbp_index, fbr_index}] = CBIR_Qsim(Relevant_Index, Irrelevant_Index, Pre_Distance);
...
Time_MassDist_Query = Time_avgDistance(1) + Paras.mtime(2) / ins_num;
Time_MassDist_RF = Time_avgDistance(F_num);
Time_add = zeros(1, fb_round + 1);
Time_add(1) = Time_MassDist_Query;
Time_add(2:end) = Time_MassDist_RF;
...
savename = strcat(resultpath, 'Results_', methodname, '_', dataname, '_', num2str(NumTree), '_', num2str(NumSub), '_', num2str(NumDim), '.mat');
