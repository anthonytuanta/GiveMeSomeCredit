function results_ttest = Main_Retrieval_ttest(filepath, dataname, methodname1, methodname2, round, Paras)
% paired t-test for retrieval results in relevance feedback
...
Result_BEP_1 = zeros(NumClass, query_num, fb_process);
Result_effectiveness_1 = zeros(NumClass, query_num, fb_process);
Result_avePrecision_1 = zeros(NumClass, query_num, fb_process);
Result_TopPrecision_1 = zeros(NumClass, query_num, fb_process);

for class_index = 1:NumClass
    for query_index = 1:query_num
        temp = Result_BEP{class_index, query_index};
        Result_BEP_1(class_index, query_index, :) = ...
            [temp{1, round + 1}, temp{2, round + 1}, temp{3, round + 1}, temp{4, round + 1}, temp{5, round + 1}];
        temp = Result_effectiveness{class_index, query_index};
        Result_effectiveness_1(class_index, query_index, :) = ...
            [temp{1, round + 1}, temp{2, round + 1}, temp{3, round + 1}, temp{4, round + 1}, temp{5, round + 1}];
        temp = Result_avePrecision{class_index, query_index};
        Result_avePrecision_1(class_index, query_index, :) = ...
            [temp{1, round + 1}, temp{2, round + 1}, temp{3, round + 1}, temp{4, round + 1}, temp{5, round + 1}];
        temp = Result_TopPrecision{class_index, query_index};
        Result_TopPrecision_1(class_index, query_index, :) = ...
            [temp{1, round + 1}, temp{2, round + 1}, temp{3, round + 1}, temp{4, round + 1}, temp{5, round + 1}];
    end
end
...
Result_BEP_2 = zeros(NumClass, query_num, fb_process);
Result_effectiveness_2 = zeros(NumClass, query_num, fb_process);
Result_avePrecision_2 = zeros(NumClass, query_num, fb_process);
Result_TopPrecision_2 = zeros(NumClass, query_num, fb_process);

for class_index = 1:NumClass
    for query_index = 1:query_num
        temp = Result_BEP{class_index, query_index};
        Result_BEP_2(class_index, query_index, :) = ...
            [temp{1, round + 1}, temp{2, round + 1}, temp{3, round + 1}, temp{4, round + 1}, temp{5, round + 1}];
        temp = Result_effectiveness{class_index, query_index};
        Result_effectiveness_2(class_index, query_index, :) = ...
            [temp{1, round + 1}, temp{2, round + 1}, temp{3, round + 1}, temp{4, round + 1}, temp{5, round + 1}];
        temp = Result_avePrecision{class_index, query_index};
        Result_avePrecision_2(class_index, query_index, :) = ...
            [temp{1, round + 1}, temp{2, round + 1}, temp{3, round + 1}, temp{4, round + 1}, temp{5, round + 1}];
        temp = Result_TopPrecision{class_index, query_index};
        Result_TopPrecision_2(class_index, query_index, :) = ...
            [temp{1, round + 1}, temp{2, round + 1}, temp{3, round + 1}, temp{4, round + 1}, temp{5, round + 1}];
    end
end
