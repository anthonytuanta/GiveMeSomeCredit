function Main_Regression_RandMassSVR(filepath, dataname_all, Paras)
% Mass-epsilon-SVR method
...
for data_index = 1:NumData
    ...
    for r = 1:rounds
        ...
        if strcmpi(methodname, 'RMepSVR')  || strcmpi(methodname, 'RMnuSVR')
            MassModel = MassTrain(Data_tr, NumT, NumSub, HLevel, rseed(r));
            mtime(r, 1) = MassModel.ElapseTime;
            [Data_Mass, mtime(r, 2)] = MassPredict(Data, MassModel);
            clear MassModel;
        elseif strcmpi(methodname, 'RipMepSVR') || strcmpi(methodname, 'RipMnuSVR')
            MassModel = MassIntPoTrain(Data_tr, NumT, NumSub, HLevel, rseed(r));
            mtime(r, 1) = MassModel.ElapseTime;
            [Data_Mass, mtime(r, 2)] = MassIntPoPredict(Data, MassModel);
            clear MassModel;
        else
            ...
            for g_index = 1:length(glist)
                ...
                for c_index = 1:length(clist)
                    for e_index = 1:length(elist)
                        ...
                        if mykernel
                            if strcmpi(methodname, 'RMepSVR') || strcmpi(methodname, 'RipMepSVR')
